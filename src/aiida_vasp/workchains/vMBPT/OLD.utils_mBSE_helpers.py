...
def _convert_array2D_f(entry, dim):
    data = None
...
def _elaborate_epsilon_diag(vasprun_path):
    vasprun = ET.parse(vasprun_path)
    v = vasprun.findall('.//varray[@name="epsilon_diag"]/v')
    arr = np.asarray(_convert_array2D_f(v, 2))
    x, y = arr[:,0], arr[:,1]
    inv_eps0 = arr[np.argmin(arr[:,0]), 1]
    def model(x, a1, eps_lowestG=inv_eps0):
        return 1 - (1-eps_lowestG)*np.exp(-x**2/(4*a1**2))
    best, _ = curve_fit(model, x, y, p0=[1.26])
    return float(inv_eps0), float(best[0])

def _extract_NBANDSGW_from_vasprun(vasprun_path):
    return int(Vasprun(vasprun_path).parameters['NBANDSGW'])

def _extract_gap_from_vasprun(vasprun_path):
    BSV = BSVasprun(vasprun_path)
    return float(BSV.get_band_structure().get_band_gap()['energy'])
...
def determine_BSE_parameters( bandsdata: orm.BandsData,
    ...
    log.append("[ΔE_conb_from_CBM] (eV):\n") ;   log.append("  ".join([f"{v:7.4f}" for v in DeltaE_conb_fromCBM]) + "\n")
    ...
    output['OMEGAMAX'] =  spectra_energy_window_aboveGap 
    log.append( f"\n[result] energy_window_goal = {energy_window_goal:.2f} eV, "
