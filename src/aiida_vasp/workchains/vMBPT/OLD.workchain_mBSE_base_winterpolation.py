...
class VaspmBSEInitScriptWorkChain(WorkChain):
    ...
    def prepare_run_DFTground_NSP(self):
            ...
            ##[Part 3: Reporting]    
            msg_prefix = "  > "
            msg_prolog = f"[<{self.inputs.ns_option.calculation_label}> prepare_run_DFTground_NSP] launching DFT pk={runningWC_DFTgr_NSP.pk}"
            msg = msg_prolog +"\n"+ self.__generate_compact_submission_string( wc_node=runningWC_DFTgr_NSP , 
                                                                          prefix=msg_prefix, include_BSE_parameters=False )+"\n"
            self.report(msg)
    ...
    #
    #         inputs_DFTgr_SP.ns_reference.starting_RemoteData = self.ctx.finishedWC_DFTgr_NSP[-1].outputs.RemoteData_DFT
    #
...
#                                       "--nbandsgw_dense "  +str(args_interpolation['nbandsgw_to_interpolate']) ) 
            ...
            BSE_params_estimated = _determine_BSE_parameters( bandsdata = DFT_lastWorkchain_node.outputs.bands , 
