...
class BandsState_InterpOp :
    ...
    def _determine_BZ_IBZ_grid(bands_state, symprec=1e-5, flag_verbose=False):
        ...
        if flag_verbose:
            ...
            print("   [IBZ] Structure supplied has spacegroup = ",spglib.get_spacegroup( spglib_str , symprec=symprec))  #this as sanity chec
        ...
        spg_map, spg_grid_BZ = spglib.get_ir_reciprocal_mesh(kpts_mesh , spglib_str , is_shift=[0, 0, 0], symprec=symprec)
        ...
        def _lexsort_rows(points):
            ...
            return points[order]
        # Sanity check: the IBZ k-points picked out from the BZ list must equal the separately
        # computed IBZ list (this is pure internal consistency of the spglib output).
        assert np.all( spg_direct_BZ[spg_map_BZtoIBZ] == spg_direct_IBZ )

        # --- [3] Align spglib's IBZ with the BandsState IBZ (representative-independent) --------
        # bands_state.kpoints.kpts (read from the sparse-GW OUTCAR) and spg_direct_IBZ both
        # enumerate the SAME irreducible wedge of this mesh, but VASP and spglib generally pick
        # DIFFERENT symmetry-equivalent representatives for each star (e.g. VASP stores
        # [-1/3, 1/3, 1/6] where spglib stores [-1/3, -1/3, 1/6]). A literal coordinate comparison
        # therefore spuriously fails even though the two sets are physically identical.
        #
        # So instead of comparing coordinates, fold every BandsState IBZ k-point onto spglib's BZ
        # grid (modulo a reciprocal-lattice vector G) and read off, from spglib's own BZ->IBZ map,
        # which compact IBZ position (0..nIBZ-1) that star belongs to. This uses spglib's exact
        # equivalence relation, so it is independent of the representative convention.
        #
        # map_IBZpos_to_bandsstateIdx[p] = row of bands_state.kpoints.kpts / ibz_BS.eigenval that
        # holds the data for spglib IBZ representative p. It is required because the eigenvalues
        # are ordered as bands_state.kpoints.kpts (VASP order), NOT as spglib orders the IBZ.
        bandsstate_kpts = np.asarray(bands_state.kpoints.kpts, dtype=float)
        nIBZ = len(spg_map_BZtoIBZ)
        map_bandsstateIdx_to_IBZpos = np.full(len(bandsstate_kpts), -1, dtype=int)
        for b_idx, kb in enumerate(bandsstate_kpts):
            fold_diff  = spg_direct_BZ - kb
            fold_diff -= np.rint(fold_diff)                 # reduce modulo reciprocal-lattice vectors
            bz_hits = np.where(np.all(np.abs(fold_diff) < 1E-4, axis=1))[0]
            if len(bz_hits) == 1:
                map_bandsstateIdx_to_IBZpos[b_idx] = np.where(spg_map[bz_hits[0]] == spg_map_BZtoIBZ)[0][0]

        # The mapping must be a bijection onto {0..nIBZ-1}: every BandsState IBZ point lands on
        # exactly one grid point, and no two belong to the same star. Anything else means the
        # structure or mesh passed to spglib does not correspond to the k-points in the OUTCAR.
        if (len(bandsstate_kpts) != nIBZ
                or np.any(map_bandsstateIdx_to_IBZpos < 0)
                or sorted(map_bandsstateIdx_to_IBZpos.tolist()) != list(range(nIBZ))):
            # Only printed on failure - dump both lists so a mismatch can be diagnosed directly
            # from the scheduler output instead of needing to reproduce locally.
            print("DEBUG [_determine_BZ_IBZ_grid] could not match bands_state IBZ to spglib IBZ:")
            print(f"  bands_state.kpoints.kpts (lexsorted), shape={bandsstate_kpts.shape}:")
            print(_lexsort_rows(bandsstate_kpts))
            print(f"  spglib spg_direct_IBZ (lexsorted), shape={spg_direct_IBZ.shape}:")
            print(_lexsort_rows(spg_direct_IBZ))
            print("  bands_state row -> spglib IBZ position (-1 = unmatched):")
            print(map_bandsstateIdx_to_IBZpos)
            raise AssertionError("ERROR: The kpoints saved in self.kpoints could not be matched "
                                  "(modulo symmetry) to the IBZ returned by spglib.")

        # Invert: for each spglib IBZ position p, which BandsState row stores its data.
        map_IBZpos_to_bandsstateIdx = np.empty(nIBZ, dtype=int)
        map_IBZpos_to_bandsstateIdx[map_bandsstateIdx_to_IBZpos] = np.arange(len(bandsstate_kpts))
    ...
    # - map_IBZpos_to_bandsstateIdx: for each spglib IBZ position (0..nIBZ-1), the row of
    #     bands_state.kpoints.kpts / ibz_BS.eigenval holding its data (size nIBZ). Needed because
    #     spglib and VASP order the IBZ (and pick representatives) differently.
        return spg_map_BZtoIBZ , spg_map_IBZtoBZ_symConnected , spg_map_IBZtoBZmodified_symConnected , spg_direct_IBZ , spg_direct_BZ , spg_direct_BZmodified , map_IBZpos_to_bandsstateIdx
    ...
    def _map_bands_fromIBZ_toEdgedBZ( ibz_BS: BandsState , flag_verbose: bool = True ) -> BandsState:
        ...
        (spg_map_BZtoIBZ,
         ...
         spg_direct_IBZ,
         spg_direct_BZ,
         spg_direct_BZmodified,
         map_IBZpos_to_bandsstateIdx) = bz_mapping
        ...
        for sp in ibz_BS.spin_keys:
            ...
            for map_value_Idx, map_value in enumerate(spg_map_IBZtoBZmodified_symConnected):
                # map_value is a spglib IBZ position; translate it to the matching BandsState row
                # (VASP order) before pulling eigenvalues/occupations, which are stored in that order.
                ibz_row = map_IBZpos_to_bandsstateIdx[map_value]
                bz_eigenval[sp][map_value_Idx, :] = ibz_BS.eigenval[sp][ibz_row, :]
                bz_occupation[sp][map_value_Idx, :] = ibz_BS.occupation[sp][ibz_row, :]
        ...
        #[5] Cross-check NEW vs OLD mapping (diagnostic only) ===
        # (added 2026-07) The mapping above is the corrected, representative-independent one.
        # For transparency we ALSO run the superseded _OLD_map_bands_fromIBZ_toEdgedBZ and print
        # whether the two BZ-mapped results are identical. We ALWAYS return the NEW (corrected)
        # result; the OLD one is used only to emit an agreement verdict. The OLD routine may
        # legitimately raise (its strict IBZ assertion) or disagree (its eigenvalue indexing
        # ignores the VASP-vs-spglib IBZ ordering) - both outcomes are reported, never fatal.
        BandsState_InterpOp._crosscheck_map_bands_OLDvsNEW( ibz_BS , bz_BS , flag_verbose=flag_verbose )
        return bz_BS

    # =====================================================================================
    # SUPERSEDED ORIGINALS - kept (per request) for side-by-side cross-checking ONLY.
    # These are the pre-2026-07 versions. They are NOT used to produce the interpolation; the
    # corrected _determine_BZ_IBZ_grid / _map_bands_fromIBZ_toEdgedBZ above are what runs.
    # _crosscheck_map_bands_OLDvsNEW (below) executes _OLD_map_bands_fromIBZ_toEdgedBZ and reports
    # whether it agrees with the corrected result.
    #
    # WHY THEY WERE SUPERSEDED (two independent defects):
    #   (1) _OLD_determine_BZ_IBZ_grid asserted that bands_state.kpoints.kpts and spglib's IBZ
    #       were COORDINATE-identical (after lexsort). VASP and spglib pick different-but-
    #       symmetry-equivalent IBZ representatives for many stars, so this assertion raises even
    #       when the two sets are physically identical. This is exactly what aborted the 6x6x6
    #       I-4 run with "kpoints ... differ".
    #   (2) Even if that assertion were bypassed, _OLD_map_bands_fromIBZ_toEdgedBZ indexed the
    #       eigenvalue array - which is stored in VASP / self.kpoints order - directly with a
    #       *spglib* IBZ position. Whenever spglib orders the IBZ differently from VASP (the
    #       normal case) this silently copies the wrong k-point's energies onto the BZ grid.
    # The corrected code fixes both: it matches BandsState IBZ points to spglib's IBZ via spglib's
    # OWN equivalence relation (fold onto the BZ grid, read spg_map), then remaps each spglib IBZ
    # position back to the correct BandsState row before copying eigenvalues.
    # =====================================================================================
    @staticmethod
    def _crosscheck_map_bands_OLDvsNEW( ibz_BS: BandsState , new_bz_BS: BandsState , flag_verbose: bool = True ) -> bool:
        """
        Diagnostic: run the superseded _OLD_map_bands_fromIBZ_toEdgedBZ and compare its BZ-mapped
        bands element-wise with the NEW (corrected) result `new_bz_BS`.

        Returns True iff OLD and NEW agree. NEVER raises: if the OLD routine itself fails (e.g. its
        strict IBZ assertion), that is caught and reported as "methods differ". The caller always
        uses the NEW result for the interpolation regardless of the verdict.
        """
        import io, contextlib
        tag = "{<CROSS-CHECK OLD vs NEW BZ mapping>}"
        try:
            # The OLD routine prints unconditionally (nova "ADDING"/"nope" lines); silence those so
            # only the verdict below reaches the log. Any exception still propagates to the except.
            with contextlib.redirect_stdout(io.StringIO()):
                old_bz_BS = BandsState_InterpOp._OLD_map_bands_fromIBZ_toEdgedBZ( ibz_BS , flag_verbose=False )
        except Exception as exc:
            print(f"{tag} OLD routine could NOT run ({type(exc).__name__}: {exc})")
            print(f"{tag} -> OLD and NEW DIFFER (OLD unavailable); using NEW (corrected) result.")
            return False

        # Both build the BZ list from the same spglib call, so the BZ k-point ORDER is identical
        # and a direct element-wise comparison is meaningful.
        agree = True ; details = []
        new_kpts = np.asarray(new_bz_BS.kpoints.kpts, dtype=float)
        old_kpts = np.asarray(old_bz_BS.kpoints.kpts, dtype=float)
        if new_kpts.shape != old_kpts.shape or not np.allclose(new_kpts, old_kpts, atol=1E-8):
            agree = False ; details.append(f"BZ k-point lists differ (shapes {old_kpts.shape} vs {new_kpts.shape})")
        for sp in ibz_BS.spin_keys:
            for name, new_arr, old_arr in (("eigenval",   new_bz_BS.eigenval[sp],   old_bz_BS.eigenval[sp]),
                                           ("occupation", new_bz_BS.occupation[sp], old_bz_BS.occupation[sp])):
                new_arr = np.asarray(new_arr) ; old_arr = np.asarray(old_arr)
                if new_arr.shape != old_arr.shape:
                    agree = False ; details.append(f"{name}[{sp}] shape {old_arr.shape} vs {new_arr.shape}")
                elif not np.allclose(new_arr, old_arr, atol=1E-6, equal_nan=True):
                    agree = False
                    n_diff = int(np.sum(~np.isclose(new_arr, old_arr, atol=1E-6, equal_nan=True)))
                    maxabs = float(np.nanmax(np.abs(new_arr - old_arr)))
                    details.append(f"{name}[{sp}]: {n_diff} entries differ, max|Δ|={maxabs:.3e}")
        if agree:
            print(f"{tag} MATCH: OLD and NEW produce identical BZ-mapped bands.")
        else:
            print(f"{tag} DIFFER: OLD and NEW disagree (NEW is the corrected mapping and is the one used).")
            for d in details:
                print("   -", d)
        return agree

    @staticmethod
    def _OLD_determine_BZ_IBZ_grid(bands_state, symprec=1e-5, flag_verbose=False):
        """
        [SUPERSEDED 2026-07 - retained ONLY for the OLD-vs-NEW cross-check; use the corrected
        _determine_BZ_IBZ_grid instead. See the section banner above for why.]
        ...
        if flag_verbose:
            print("   [IBZ] Determining full BZ grid and IBZ->BZ mapping:")
            print("   [IBZ] Structure supplied has spacegroup = ",spglib.get_spacegroup( spglib_str , symprec=symprec))
        ...
        spg_direct_IBZ  = spg_direct_BZ[spg_map_BZtoIBZ]
        spg_map_IBZtoBZ_symConnected = np.array( [np.where(map_value == spg_map_BZtoIBZ)[0][0]  for map_value in spg_map])
        # Sanity checks (ORIGINAL, strict): require the two IBZ lists to be coordinate-identical.
        # NOTE: this is the check that (incorrectly) aborts when VASP and spglib pick different
        # symmetry-equivalent representatives - see the corrected routine for the fix.
        def _lexsort_rows(points):
            points = np.asarray(points)
            order = np.lexsort(points.T[::-1])
            return points[order]
        ...
        if (_sorted_bandsstate_kpts.shape != _sorted_spglib_ibz_kpts.shape
            ...
            print("DEBUG [_OLD_determine_BZ_IBZ_grid] kpoints mismatch between bands_state and spglib IBZ:")
        ...
        # Augment BZ with zone-boundary images (k and k+G), exactly as the original did.
        spg_map_modified       = deepcopy(spg_map).tolist()
        spg_direct_BZmodified  = deepcopy(spg_direct_BZ).tolist()
        kBZ_onBZedge =[]  ;  kBZ_onBZedge_mapIdx = []
        for kBZ_mapIdx , kBZ in  zip(spg_map , spg_direct_BZ):
    ...
    @staticmethod
    def _OLD_map_bands_fromIBZ_toEdgedBZ( ibz_BS: BandsState , flag_verbose: bool = True ) -> BandsState:
        """
        [SUPERSEDED 2026-07 - retained ONLY for the OLD-vs-NEW cross-check; use the corrected
        _map_bands_fromIBZ_toEdgedBZ instead.]

        Original IBZ->BZ expansion. Copies IBZ eigenvalues to the BZ grid indexing the eigenvalue
        array (stored in VASP order) directly with a spglib IBZ position - which is the defect (2)
        described in the section banner above.
        """
        if flag_verbose:
            print("{<MAPPING bands IBZ -> BZ>} <------------------>")

        #[1] Determine BZ/IBZ grids and mappings (ORIGINAL strict routine)
        bz_mapping = BandsState_InterpOp._OLD_determine_BZ_IBZ_grid( ibz_BS , flag_verbose=flag_verbose )
        (spg_map_BZtoIBZ,
         spg_map_IBZtoBZ_symConnected,
         spg_map_IBZtoBZmodified_symConnected,
         spg_direct_IBZ,
