...
def _getKmesh_from_Kspacing(latVec , KSPACING , flag_round=True):
        ...
        Rk = 2 * np.pi / KSPACING
        if flag_round:  Kmesh = [ int( np.round( max( 1 , Rk* rec_cell_norm[x]) ) ) for x in range(3)]
        else:           Kmesh = [ max( 1 , Rk* rec_cell_norm[x]) for x in range(3)]
        return Kmesh
...
class VaspG0W0KptsConvWorkChain(WorkChain):
    ...
    def define(cls, spec):
            ...
            spec.input( 'ns_kpoints.kDensity_minimum_constraint' , valid_type=Float , required=False , default=lambda:Float(0.2),help="constraint on the minimum k-point density - that converged k-mesh must guarantee." )
            spec.input( 'ns_kpoints.kDensity_startingValue'      , valid_type=Float , required=False , default=lambda:Float(0.3), help="starting value for the k-point density convergence." ) 
            spec.input( 'ns_kpoints.kDensity_maxValue'           , valid_type=Float , required=False , default=lambda:Float(1.1), help="maximum value for the k-point density convergence." ) 
            spec.input( 'ns_kpoints.kDensity_step'               , valid_type=Float , required=False , default=lambda:Float(0.1), help="minimum constraint that the converged k-mesh must guarantee." ) 
            spec.input( 'ns_kpoints.convergence_threshold'       , valid_type=Float , required=False , default=lambda:Float(0.1), help="minimum constraint that the converged k-mesh must guarantee." ) 
    ...
    def initialize(self):
        kspacing_start = max( self.inputs['ns_kpoints']['kDensity_minimum_constraint'].value , self.inputs['ns_kpoints']['kDensity_startingValue'].value )
        kspacing_max   = max( self.inputs['ns_kpoints']['kDensity_maxValue'].value , kspacing_start) 
        ...
        self.ctx.list_ks_kmesh = np.insert(np.array(list_kmesh , dtype=float) , 0 , list_kspacing , axis=1)
        #self.ctx.list_ks_kmesh[idx,0] = k-point density of idx entry ; self.ctx.list_ks_kmesh[idx,1:] = k-point mesh of idx entry
        
        _ , list_ks_kmesh_uniqueidx   = np.unique( self.ctx.list_ks_kmesh[:,1:] , axis=0 , return_index=True )      
        self.ctx.list_ks_kmesh_unique = self.ctx.list_ks_kmesh[list_ks_kmesh_uniqueidx , :]
        ...
        self.ctx.control['Delta']= AttributeDict()
        self.ctx.control['Delta']['kdensity_notRounded'] = [_getKspacing_from_Kmesh(self.inputs.structure.cell , kmesh ) for kmesh in self.ctx.list_ks_kmesh_unique[:,1:]] 
        ...
        self.ctx.control.Delta_GWgap    = AttributeDict() ; self.ctx.control.Delta_GWgap['spinUp']    = AttributeDict() ;
        ###self.ctx.control.Gradient_GWgap = AttributeDict() 
        if ('magnetic_moment_onsite' in self.inputs['ns_parameters']):  
            self.ctx.control.Delta_GWgap['spinDw']    = AttributeDict() ;


        if self.inputs.ns_option.verbose:
            self.report("\n [Initializing K-points convergence]"+
                        "\n [input] ns_kpoints.kDensity_minimum_constraint:"+str(self.inputs.ns_kpoints.kDensity_minimum_constraint)+
                        "\n [input] ns_kpoints.kDensity_startingValue:     "+str(self.inputs.ns_kpoints.kDensity_startingValue)     +
                        "\n [input] ns_kpoints.kDensity_step:              "+str(self.inputs.ns_kpoints.kDensity_step)              +                        
                        "\n [input] ns_kpoints.convergence_threshold:      "+str(self.inputs.ns_kpoints.convergence_threshold)+" eV"+
                        "\n > kDensities to-test: "+str(self.ctx.list_ks_kmesh_unique[:,0]) +
                        "\n > corresponding to:   "+np.array2string(self.ctx.list_ks_kmesh_unique[:,1:] , separator=" , ").replace('\n', ''))
    ...
    def prepare_run_DFTgr_NSP(self):
        ...
        self.ctx.inputs_DFTgr_NSP.kpoints.set_kpoints_mesh(  np.array(  self.ctx.list_ks_kmesh_unique[self.ctx.control.iteration_counter,1:] , dtype=int))
        ...
        if  ("DFTgr_RemoteData" not in self.inputs["ns_reference"]): # (non-Spin-polarized from scratch)
            self.report("\n [wkc_KptsConv][Preliminary DFT-GroundState calc - NonSpinPolarized - From scratch]\n               > Lauching DFT(NonSpinPolarized) using workchain_DFT_G0W0 on k-mesh "
                        +np.array2string(self.ctx.list_ks_kmesh_unique[self.ctx.control.iteration_counter,1:] , separator=" , ").replace('\n', '')+"\n")
    ...
    def prepare_run_DFT_G0W0(self):
        ...
        self.ctx.input_DFTG0W0.kpoints.set_kpoints_mesh(  np.array(  self.ctx.list_ks_kmesh_unique[self.ctx.control.iteration_counter,1:] , dtype=int))
    ...
    def monitor_convergence(self): 
        ...
        if self.ctx.control.iteration_counter >= ( len(self.ctx.list_ks_kmesh_unique) - 1) :
    ...
    def elaborate_results(self):
            ...
            self.out( 'final_kDensity' , self.ctx.list_ks_kmesh_unique[self.ctx.control.iteration_counter,0]  )
            self.out( 'final_kMesh'    , self.ctx.list_ks_kmesh_unique[self.ctx.control.iteration_counter,1:] )
