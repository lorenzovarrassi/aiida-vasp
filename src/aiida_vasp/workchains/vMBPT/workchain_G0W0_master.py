...
class VaspG0W0CompleteWorkChain(WorkChain):
    ...
    def prepare_run_DFT_G0W0_dense(self):
            #Guard: the basis-set extrapolation workchain must have finished successfully before its
            #outputs (pairs_nbands_encuts, used below) can be read; otherwise fail cleanly here instead
            #of crashing on a missing output attribute.
            if not self.ctx.finishedWC_extrBS[-1].is_finished_ok:
