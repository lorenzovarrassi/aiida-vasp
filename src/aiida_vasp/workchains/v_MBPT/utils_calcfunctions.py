...
@calcfunction
def get_closest_EncutNband_multiple(kpoints , structure , NGdata , DFTgr_ENMAXmax , param_quad , mpi_thr_number , original_value , input_type , verbose=lambda:Bool(True) , flag_twoSidesRounding=lambda:Bool(True) ):
    """
    ...
    The original value is passed to original_value; the type of the original value (if it represents a cutoff or a nbands) is determined by input_type.
    ...
    input_type : Str
        ...
        if Str("nbands") is passed, original_value is considered as a NBANDS variable; if Str("encut") is passed, is considered as a cutoff.
    verbose : TYPE, optional
        DESCRIPTION. The default is lambda:Bool(True).
    flag_twoSidesRounding : TYPE, optional
        DESCRIPTION. The default is lambda:Bool(True).
    ...
    NG          = np.array( NGdata.get_array('NGarray') , dtype = int)
    param_array = np.array( param_quad.get_list() )
    total_num_mpithread = mpi_thr_number.value
  
    ##[Step 0]
    if input_type == "encut":
        encut_0_original = original_value.value
    ...
    elif input_type == "nbands":
        nband_0_original = original_value.value
    ...
    #[Step 1]Determing the multiple of total_num_mpithread closest to nband_0_original - first pass (VASP impose a constraint on NBANDS, as it must be a multiple of total core number)
    ...
    if nband_0_original % total_num_mpithread == 0 :   #nband_0_original is a perfect multiple of total_num_mpithread
        nband_1_closestMultiple = nband_0_original
    elif flag_twoSidesRounding : nband_1_closestMultiple = round(nband_0_original / total_num_mpithread) * total_num_mpithread   #can both round up and round down
    else :                       nband_1_closestMultiple = (nband_0_original // total_num_mpithread + 1) * total_num_mpithread   #always round up
    ...
    if not encut_1_CBH == encut_1_CBH:
        linInt_array_ENCUT = np.multiply( np.array([0.6 , 0.75 , 1.00 , 1.25 , 1.587 , 1.8]) , DFTgr_ENMAXmax.value)  #larger than EMAX_fraction because value can be rounded outside usual Encut-Nbands values, introducing errors.
    ...
    if not flag_More_precise_values:
        ...
        string_log=("\n  [2.1 routine get_closest_EncutNband_multiple]"
        +"\n   nband closest multiple of #mpi -> encut under Complete Basis Hyp."
        +"\n   >  total_num_mpithread  :"+str(total_num_mpithread)
        +"\n   >  [0] Not corrected (encut , nband)   : ("+str(encut_0_original)+" , "+str(nband_0_original)+")"
        +"\n   -> [1] Corrected nband (multiple #mpi)      :"+str(nband_1_closestMultiple)
        +"\n   -> [1] encut under CBH from corrected nband :"+str(encut_1_CBH)
        +"\n          (fit - param_array :"+str(param_array)+")"
        +"\n   -> [2] nbands - refining from linear interpol. :"+str(nband_2_closestMultiple)
        +"\n   -> [2] encut  - refining from linear interpol. :"+str(encut_2_CBH)
        +"\n          CBH nbands- x data for lin.interp:"+str(linInt_array_nb)+")"
        +"\n          CBH encut - y data for lin.interp:"+str(linInt_array_encut)+")")
 
    output_array = AttributeDict()
    output_array.encut  = encut_2_CBH
    output_array.nbands = nband_2_closestMultiple
    return List([ output_array , string_log ])
#def get_closest_EncutNband_multiple(kpoints , structure , NGdata , DFTgr_ENMAXmax , param_quad , mpi_thr_number , original_value , input_type , verbose=True , flag_twoSidesRounding=True ):

@calcfunction
def get_EncutNbandFitParams_completeBasis_quadratic(kpoints , structure , NGdata , maxENMAX , verbose=Bool(True)):        
    """
    ...
    kptsArray   = kpoints.get_kpoints()
    NG          = np.array( NGdata.get_array('NGarray') , dtype = int)
    Encut_array = np.multiply([0.9 , 1.25 , 1.5] , maxENMAX.value ) # To determine the fit we use three points, corresponding to the memory conserving procedure; this is a bit arbitrary, CAN BE IMPROVED.
